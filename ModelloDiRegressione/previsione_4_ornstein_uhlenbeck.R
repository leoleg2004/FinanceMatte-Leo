# ==============================================================================
# MODELLO 4: ORNSTEIN-UHLENBECK (Mean Reverting)
# ==============================================================================
# Modella un principio economico forte: se i mercati salgono troppo, vengono
# attratti verso il basso dalla "gravità" della loro media storica, e viceversa.
...
# Parametri Mean Reverting
THETA <- mu_mensile  # La media di lungo termine verso cui si viene attratti
KAPPA <- 0.5         # Velocità di attrazione verso la media
...
cat("=========================================================================\n")
cat("   PARAMETRI DEL MODELLO ORNSTEIN-UHLENBECK\n")
cat("=========================================================================\n")
cat(sprintf("Media Attrattiva (Theta):            %.3f %%\n", THETA * 100))
cat(sprintf("Forza di Attrazione (Kappa):         %.2f\n", KAPPA))
cat(sprintf("Volatilità Aggiuntiva (Sigma):       %.3f %%\n", sigma_mensile * 100))
cat("=========================================================================\n\n")

SIGMA <- sigma_mensile

# --- MOTORE STOCASTICO: ORNSTEIN-UHLENBECK ---
...
cat("Avvio Modello ORNSTEIN-UHLENBECK: Ritorno Verso la Media...\n")
for(sim in 1:NUM_SIMULAZIONI) {
  rendimenti <- numeric(mesi_futuri)
  rendimenti[1] <- mu_mensile
  z <- rnorm(mesi_futuri)
  
  for(m in 2:mesi_futuri) {
    # Il rendimento viene "tirato" verso THETA
    rendimenti[m] <- rendimenti[m-1] + KAPPA * (THETA - rendimenti[m-1]) + SIGMA * z[m]
  }
  
  for(m in 1:mesi_futuri) {
    matrice_simulazioni[m+1, sim] <- matrice_simulazioni[m, sim] * (1 + rendimenti[m]) + VERSAMENTO_MENSILE
...
fan_chart <- ggplot() + geom_line(data=df_spaghetti, aes(x=Mese, y=Capitale, group=Simulazione), color="gray80", alpha=0.3) +
  ...
  labs(title="Modello 4: Ornstein-Uhlenbeck (Mean Reverting)", subtitle="I rendimenti estremi vengono riassorbiti dalla media", y="Capitale Reale (€)") + theme_minimal()
...
grafico_gaussiana_ritorni <- ggplot(df_storico, aes(x = Rendimenti)) +
  ...
  labs(title = "Distribuzione Storica vs Curva Normale Teorica (Rossa)", subtitle = "Nell'O-U, i valori estremi vengono artificialmente ritirati verso il centro (Mean Reversion).", x = "Rendimento Mensile", y = "Densità") +
...
grafico_gaussiana_finali <- ggplot(df_finali, aes(x = Capitale_Finale)) +
  ...
  labs(title = sprintf("Distribuzione del Capitale Finale (Tra %d Anni)", ANNI_FUTURI), subtitle = "Campana tipicamente molto più schiacciata grazie al Mean Reverting.", x = "Capitale Finale Accumulato (€)", y = "Densità") + theme_minimal() + theme(legend.position = "bottom")
