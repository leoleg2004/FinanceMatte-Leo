# ==============================================================================
# MODELLO 1: BOOTSTRAP STORICO (RESAMPLING)
# ==============================================================================
# Questo modello abbandona la teoria della "Curva a Campana" (rnorm). 
# Pesca ciecamente mesi a caso dal passato vissuto dal tuo portafoglio.
# Mantiene perfetti tutti i Cigni Neri, le fat tails e l'asimmetria reale del mercato.
...
prezzi_allineati <- dataset_grezzo %>%
  ...
  pivot_wider(names_from = Ticker, values_from = Price) %>% drop_na() %>% arrange(Date)
...
# Vettore crudo del passato (La "scatola" da cui pescare)
ritorni_reali_passato <- ritorni_mensili$Port_Return

# --- STATISTICHE FONDAMENTALI DEL PASSATO E DEL MODELLO ---
cat("\n=========================================================================\n")
cat("   ESTRAZIONE COMPORTAMENTO STORICO DEL PORTAFOGLIO\n")
cat("=========================================================================\n")
cat(sprintf("Rendimento Medio Mensile (Storico):  %.3f %%\n", mean(ritorni_reali_passato) * 100))
cat(sprintf("Volatilità Mensile (Rischio):        %.3f %%\n", sd(ritorni_reali_passato) * 100))
cat(sprintf("Asimmetria (Skewness):               %.3f  (Normale = 0)\n", calc_skewness(ritorni_reali_passato)))
cat(sprintf("Code Grasse (Kurtosis):              %.3f  (Normale = 3)\n", calc_kurtosis(ritorni_reali_passato)))
cat("=========================================================================\n\n")

cat("=========================================================================\n")
cat("   PARAMETRI DEL MODELLO\n")
cat("=========================================================================\n")
cat("Motore Bootstrap: Resampling dei dati storici. Nessun parametro stocastico inserito.\n")
cat("=========================================================================\n\n")
# --- MOTORE STOCASTICO: BOOTSTRAP ---
...
cat("Avvio Modello BOOTSTRAP: Resampling Storico...\n")
for(sim in 1:NUM_SIMULAZIONI) {
  # Invece di rnorm, peschiamo a caso dal vettore reale!
  rendimenti_futuri <- sample(ritorni_reali_passato, size = mesi_futuri, replace = TRUE)
  
  for(m in 1:mesi_futuri) {
    matrice_simulazioni[m+1, sim] <- matrice_simulazioni[m, sim] * (1 + rendimenti_futuri[m]) + VERSAMENTO_MENSILE
...
fan_chart <- ggplot() +
  ...
  labs(title="Modello 1: Bootstrap Storico (Cigni Neri Inclusi)", subtitle=sprintf("Proiezione a %d Anni al netto dell'inflazione", ANNI_FUTURI), y="Capitale Reale (€)") + theme_minimal()
...
grafico_gaussiana_ritorni <- ggplot(df_storico, aes(x = Rendimenti)) +
  ...
  labs(title = "Distribuzione Storica vs Curva Normale Teorica (Rossa)", subtitle = "Nel Bootstrap l'istogramma azzurro è usato DIRETTAMENTE, ignorando la curva rossa.", x = "Rendimento Mensile", y = "Densità") +
...
grafico_gaussiana_finali <- ggplot(df_finali, aes(x = Capitale_Finale)) +
  ...
  labs(title = sprintf("Distribuzione del Capitale Finale (Tra %d Anni)", ANNI_FUTURI), x = "Capitale Finale Accumulato (€)", y = "Densità") + theme_minimal() + theme(legend.position = "bottom")
